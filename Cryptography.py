...
def RandomPrime(bits,prime = 0): #this function returns a random prime that is larger than 258
    while prime < 258: # While prime isn't large enough for RSA...
        prime = number.getPrime(1024) # Change prime to random value created through getPrime
    return(prime) # Return that prime

#Coded using https://gist.github.com/JonCooperWorks/5314103 as a reference implementation
...
def encrypt(PublicKey, Max, plaintext):
    ...
    for character in plaintext: #breaks up plaintext into a series of characters and then for each of them ...
        ...
        # less than Max.
        ciphertext.append(CipherChar) # Attach the encrypted character to the ciphertext
    return(ciphertext)

def decrypt(PrivateKey, Max, ciphertext):
    CipherChars = ciphertext # variable rename for future compatibility
    plaintext = []
...
# PrivateKey = 
# ciphertext = encrypt(PublicKey,Max,input())
# print(ciphertext)
# print(Num2Text(decrypt(PrivateKey,Max,ciphertext)))