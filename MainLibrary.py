...
def AcceptInvite(ContactName,FileName):
    ...
    SendMessage(FileName,MessageContents,IDpassword,PublicKey,Max,ContactID,PublicKeyID)
...
def SendMessage(FileName,message,IDpassword,PublicKey,Max,ContactID,PublicKeyID):
    ...
    MyPublicKeyID, MyPublicKey, MyMax = CurrentKeyDetails(ContactID)
...
def GetMessageText(MessageID, PrivateKeyID,Max):
    contents = lsb.reveal("Messages/"+str(MessageID)+".png") # DEV NOTE: change once at rest encryption implemented
    ...
    print(MessageText)
    if temp == MessageText:
        print("You should be seeing this")
